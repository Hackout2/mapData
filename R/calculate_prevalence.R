...
calculate_prevalence <- function(data, pops = NULL, conf.level = 0.95, region.head = "region") {
    ...
    if (is.null(pops)) 
        pops <- data.frame(region = unique(data[order(data[, regioncol]), regioncol]), 
            population = ceiling(as.numeric(table(data$region)) * (rlnorm(length(unique(data$region))) + 
                1)))
    ################################## 
    ...
    prev$lower <- qnorm((1 - conf.level)/2, prev$prevalence, sqrt(prev$prevalence * 
        (1 - prev$prevalence)/prev$population))
    prev$upper <- qnorm((1 + conf.level)/2, prev$prevalence, sqrt(prev$prevalence * 
        (1 - prev$prevalence)/prev$population))
    ...
    # for(i in 1:nrow(prev)) { prev$lower[i] <- ( binom.test( prev$cases[i],
    # prev$population[i], conf.level = conf.level) )$conf.int[1] prev$upper[i] <- (
    # binom.test( prev$cases[i], prev$population[i], conf.level = conf.level)
    # )$conf.int[2] }
    return(prev)
} 
